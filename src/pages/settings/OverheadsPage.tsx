...
function OverheadsContent() {
  ...
  const openEditDialog = (overhead: Overhead) => {
    ...
    setFormData({
      ...
      start_date: overhead.start_date || "",
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.name.trim() || isNaN(parsedAmount) || parsedAmount < 0) {
      return;
    }
  ...
  const getCategoryColor = (category: OverheadCategory) => {
    const colors: Record<OverheadCategory, string> = {
  ...
  const getFrequencyLabel = (frequency: OverheadFrequency) => {
    const labels: Record<OverheadFrequency, string> = {
      daily: "Daily",
      weekly: "Weekly",
      monthly: "Monthly",
    };
  ...
  return (
    <PageLayout
      title="Overheads"
      description="Saved fixed costs used for profit and break-even calculations"
    >
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>
              {overheads?.length || 0} overhead{overheads?.length !== 1 ? "s" : ""} configured
            </span>
          </div>
              ...
              <TableBody>
                {overheads?.map((overhead) => (
                  <TableRow key={overhead.id}>
                    ...
                    <TableCell>{getFrequencyLabel(overhead.frequency)}</TableCell>
                    <TableCell className="text-right font-mono">
                      {formatCurrency(overhead.amount)}
                    </TableCell>
                    <TableCell>
                      {overhead.location_id ? (
                        <span className="text-sm">{overhead.locations?.name || "Unknown"}</span>
                      ) : (
                    ...
                    <TableCell>
                      <div className="flex items-center gap-1">
                        <Button
                          variant="ghost"
                          size="icon"
                          onClick={() => openEditDialog(overhead)}
                        >
                        ...
                        <Button
                          variant="ghost"
                          size="icon"
                          onClick={() => setDeleteId(overhead.id)}
                        >
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>
              {editingOverhead ? "Edit Overhead" : "Add Overhead"}
            </DialogTitle>
            <DialogDescription>
              {editingOverhead
                ? "Update the overhead details below."
                : "Add a new fixed cost to track in your financial calculations."}
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
                ...
                <Select
                  ...
                  onValueChange={(value: OverheadCategory) =>
              ...
              <div className="grid gap-2">
                ...
                <Select
                  ...
                  onValueChange={(value: OverheadFrequency) =>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={
                !formData.name.trim() ||
                !formData.amount ||
                createOverhead.isPending ||
                updateOverhead.isPending
              }
            >
